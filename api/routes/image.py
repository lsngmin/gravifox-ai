...
async def predict_image(
    ...
    selected_model_key: Optional[str] = Form(default=None, alias="modelKey"),
    ...
    try:
        probs = await vit_service.predict_image(
            image, model_key=selected_model_key
        )
    finally:
        image.close()
    pipeline = await vit_service.get_pipeline(model_key=selected_model_key)
